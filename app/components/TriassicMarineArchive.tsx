...
function displayAge(age: string): string {
  ...
  for (const period of TRIASSIC_MARINE_AGES) {
...
function chipHref(params: { q?: string; order?: string; age?: string }): string {
  ...
  const s = sp.toString();
  return s ? `/archive/triassic-marine?${s}` : "/archive/triassic-marine";
}

export default function TriassicMarineArchive({ q = "", order = "", age = "" }: Props) {
  const orders = getOrders();
  const ages = getAges();
  const all = searchTriassicMarine(q);
  ...
  const collectionSchema = {
    ...
    "@type": "CollectionPage",
    "@id": `${SITE_URL}/archive/triassic-marine#collection`,
    url: `${SITE_URL}/archive/triassic-marine`,
    name: "Triassic Marine Reptiles of the Luoping Biota",
    description:
      "A visual archive of 10 Triassic marine reptile species with fossil photographs, geological ages and scientific classification.",
    isPartOf: {
      "@type": "WebSite",
      "@id": `${SITE_URL}/#website`,
      name: SITE_NAME,
    ...
    mainEntity: {
      ...
      itemListElement: cards.map((c, i) => ({
        ...
        url: `${SITE_URL}/triassic-marine/${c.slug}`,
      })),
    },
  };
  ...
  return (
    <div className="tri-home-main">
      ...
      />
      <form method="get" action="/archive/triassic-marine" className="search-bar">
        <input
          type="text"
          name="q"
          defaultValue={q}
          ...
          aria-label="Search Triassic marine reptile species"
        />
      ...
      <TrilobiteGrid species={cards} detailBase="/triassic-marine" />
      ...
      <footer className="tri-footer">
        <p>
          Data compiled from the <em>Luoping Biota</em> (Middle Triassic, Yunnan, China) visual
          database. Images are for educational and reference purposes; original copyrights belong to
          the respective institutions and collectors.
        </p>
      </footer>
    </div>
  );
